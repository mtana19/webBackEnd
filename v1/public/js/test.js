...
$("td input").on("click", function(){
  if (!this.checked){
    seatss.pop($(this).val());      //add to array
  }else{
    seatss.push($(this).val());    //remove from array
  }
  $("#movieSeats").html(seatss.toString().replace(/,/g, " "));
  var cost = $("#price").val();
  var total = cost * seatss.length;
  $("#total").val(total);
});
  ...
  $('td img').click(function() {
    ...
    for(i = 0; i < seatsList.length;i++){
      if(seatsList[i] != null){
        ...
        count++;
        seatsArray.push(seatsList[i]);
      }
    }
    var cost = $("#price").val();
    var total = cost * seatsArray.length;
    $("#total").val(total);
