    ...
    for (j = 0; j < storedSchedules.length; j++) {
        ...
        if (storedSchedules[j].movie_id === movieId) {
            ...
            schedTable.innerHTML += '<tr><td>' + (++count) +
									...
									'</td><td>' + '<a href="seat_selection"><button type="button" class="btn btn-success text-white" id="' + storedSchedules[j].sched_id + '" onclick="getSchedId(this.id)">Select</button></a></td></tr>';
        }
    }    
}
